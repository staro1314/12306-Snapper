...
export async function initializeOfficialOrder(trainCode: string, queryId: string): Promise<void> { const response = await fetch("/browser/observe/order-initialize", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ trainCode, queryId, confirmObservation: true }) }); const payload = await response.json(); if (!response.ok || payload.currentPath !== "/otn/confirmPassenger/initDc") throw new Error(payload.error ?? "订单初始化未进入确认页面"); }
